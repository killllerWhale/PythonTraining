...
class Queen:
    ...
    def can_move(self, board, row, col, row1, col1):
        ...
        step = 1 if (row1 >= row) else -1
        for r in range(row + step, row1, step):
            # Если на пути по горизонтали-вертикали есть фигура
            colStep + 1 if step == 1 else colStep - 1
            if not (board.get_piece(r, colStep) is None):
                return False
        colStep = col
        ...
        step = 1 if (col1 >= col) else -1
        for c in range(col + step, col1, step):
            # Если на пути по вертикали - горизонтали есть фигура
            rowStep + 1 if step == 1 else rowStep - 1
            if not (board.get_piece(row, c) is None):
                return False
        rowStep = row
